...
import StoreRoomModal from './components/StoreRoomModal'
...
const StoreRoom = () => {
  ...
  const refresh = () => {
    placeServices.getStoreRooms([]).then((response) => {
      setStoreRoom(response)
    })
  }
  ...
  const removeItem = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        placeServices.removeStoreRoom(id)
        setTimeout(() => {
          refresh()
        }, 300)
      }
      ...
      <CCard className="mb-4">
        <CCardHeader>
          ...
          <div className="float-end">
            <StoreRoomModal refresh={refresh} action="add" id={null} object={tempObject} />
          </div>
        </CCardHeader>
        <CCardBody>
          <CRow>
