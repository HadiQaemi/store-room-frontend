...
const BranchModal = ({ refresh, action = 'add', id, object }) => {
  const [visible, setVisible] = useState(false)
  const { register, handleSubmit, setValue } = useForm()
  const onSubmit = async (data) => {
    if (action === 'add') {
      placeServices.insertBranch(data).then(() => {
        refresh()
        setVisible(false)
      })
    } else if (action === 'update') {
      placeServices.updateBranch(object.id, data).then(() => {
        refresh()
        setVisible(false)
        object = null
      })
  ...
  return (
    <>
      ...
      <CModal alignment="center" visible={visible} onClose={() => setVisible(false)}>
        <CModalHeader>
          <CModalTitle>{action === 'add' ? 'شعبه جدید' : 'ویرایش شعبه'}</CModalTitle>
        </CModalHeader>
        <CModalBody>
