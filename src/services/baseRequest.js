...
export default async function baseRequest(
  ...
  try {
    const response = await axios({
      url: `${API}${path}`,
      // url: 'https://apitester.ir/api/Categories',
      method: method === 'post' && data ? 'POST' : method,
      headers:
        method === 'get' &&
        (data ? { ...headers, 'X-HTTP-Method-Override': method.toUpperCase() } : headers),
    ...
    if (response.status >= 200 && response.status < 300) {
      ...
      response.data.message &&
        toastAnyWhere.show(response.data.message, response.data.isSuccess ? 'success' : 'error')
    ...
    if (!axios.isCancel(error)) {
      ...
      if (error.response.status === 401) {
        ...
        Toast.fire({
          icon: 'error',
          title: 'نام کاربری یا رمز عبور صحیح نمی باشد',
        })
        if (retry) {
          await authenticationServices.logout()
        }
